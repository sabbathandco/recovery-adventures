...
export const normalise = (arr) =>
  arr.map((a, idx) => {
    ...
    if (refs.length < 2) {
      const txt  = a.title ?? a.name ?? "";
      const pool = /garden|herb/i.test(txt)   ? FALLBACK.gardening :
                  /crochet|knit/i.test(txt)  ? FALLBACK.crochet   :
                  /read|book/i.test(txt)     ? FALLBACK.reading   :
                                               FALLBACK.default;
    ...
    return {
      ...
      description: a.description ?? a.overview ?? "",   // ← new line
      category,
      references : refs
    };
  });
